    ...
    public void setup() {
      ...
      //  Query q = entityManager.createQuery("SELECT x FROM User x");
        ...
        /*if (users.size() == 0) {
            ...
            users.models.User user = users.models.User.createSystemAdmin ("","");

        }  */

        User user = User.createSystemAdmin("admin@flow7.net","password1","David Dale");
        entityManager.persist( user );

        System.out.println("Just persisted user " + user.id );
